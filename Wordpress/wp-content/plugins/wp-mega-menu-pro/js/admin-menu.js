/**
 * WP Mega Menu Pro - 2.0.8 version Admin JS Script
 */
 ...
 jQuery(function ($) {
    ...
    var check_version = apmm_variable.check_version;
        ...
        success: function(res) {
         ...
         //var editorIdd = $('#wpmm_html_content_'+id);
             ...
             if(check_version == "addeditor"){
                  $("#wpmm_choose_topcontent_type option[value='html']").removeAttr('disabled');
                  $("#wpmm_choose_bottomcontent_type option[value='html']").removeAttr('disabled');
               ...
               $("#wpmm_choose_topcontent_type option[value='html']").attr('disabled','disabled');
               $("#wpmm_choose_bottomcontent_type option[value='html']").attr('disabled','disabled');
               $('.toggle_html').hide();
               $('.toggle_bhtml').hide();
             }  
...
var checked_enable_megamenu = apmm_variable.checked_enable_megamenu;
var template_check = apmm_variable.template_check;
var location_check = apmm_variable.location_check;
$('.wpmm_available_custom').click(function(){
  ...
  if(enablemegamenu == 0){
    ...
    alert(location_check);
     ...
     $.ajax({
      ...
      type: 'post',
      data: {
          action: "wpmm_show_custom_option",
          mlocation: mlocation,
      ...
      cache: false,
     complete: function() {
     //$('.wpmmpro-templates-custom-wrapper').show();
     },
    success: function(res) {
      $('.wpmmpro-templates-custom-wrapper').slideToggle();
      $('.wpmmpro-templates-custom-wrapper').html(res);
      $('.apmm-color-picker').wpColorPicker();
       $( '.wpmm-custom-options-wrap .wpmm-template-tab-leftsection' ).on('click','.wpmm_tabss', function() {
          $('.wpmm-custom-options-wrap .wpmm_tabss').removeClass('active');
          var tabidd = $(this).attr('id');

          $(this).addClass('active');
          $('.wpmm-custom-options-wrap .tab-panes').css('display','none');
          $('.wpmm-custom-options-wrap .widget_right_section #tabs_'+tabidd).css('display','block');
        });
  ...
  $('.wpmmpro-templates-custom-wrapper').on('click','span.wpmm-save-data',function(e){
          e.preventDefault();
        // retrieve the widget settings form
        var wpcsettings = JSON.stringify($( "[name^='apmegamenu_custommeta']" ).serializeArray());
              $.ajax({
                  url: AjaxUrl,
                  type: 'post',
                  data: {
                      action: "wpmmsavecustomsettings",
                      wpmenuid: $('#menu').val(),
                      wp_megamenu_cmeta: wpcsettings,
                      wp_nonce: admin_nonce
                  },
                  beforeSend: function() {
                     $(".wpmm-load-data").css('display', 'block');
                     $(".wpmm-load-data img").css('display', 'block');
                 },
                 complete: function() {
                     $(".wpmm-load-data").css('display', 'block');
                     $(".wpmm-load-data img").css('display', 'none');
                     $(".wpmm_saving_message").show();
                 },
                 success: function(res) {
                    //console.log(res);
                   $(".wpmm-load-data").css('display', 'none');
                   $(".wpmm_saving_message").html(saved_success_message).delay(2000).fadeOut('slow');
               }
           });
       });


$('.wpmmpro-templates-custom-wrapper').on('click','span.wpmm-close-wrap',function(){
  $('.wpmmpro-templates-custom-wrapper').hide();
  $('.wpmmpro-templates-custom-wrapper').find('.wpmm-custom-options-wrap').remove();
});
